...
- **Privacy policy must mirror App Store Connect App Privacy.** Current ASC analytics label declares Product Interaction, Purchase History, Health, and Device ID under `Data Not Linked to You`, each used for Analytics only and not tracking. Keep `privacy.html` aligned whenever app analytics changes.
